...
- FastMCP tool servers for:
  ...
  - Demo service health, incidents, and deployment history
...
```bash
python -m autoops.main "show health for checkout-service"
```

More examples:

```bash
python -m autoops.main "list open PRs"
python -m autoops.main "show logs for checkout-service"
python -m autoops.main "run a security scan on autoops"
python -m autoops.main "summarize the incident for checkout-service"
python -m autoops.main "create an issue for checkout-service high error rate"
```
...
```bash
python autoops/mcp/mcp_github.py
python autoops/mcp/mcp_cloudwatch.py
python autoops/mcp/mcp_codereview.py
python autoops/mcp/mcp_monitoring.py
```

To expose an MCP server over SSE, set `MCP_TRANSPORT=sse` and the relevant port variable:

```bash
MCP_TRANSPORT=sse GITHUB_MCP_PORT=8001 python autoops/mcp/mcp_github.py
MCP_TRANSPORT=sse CLOUDWATCH_MCP_PORT=8002 python autoops/mcp/mcp_cloudwatch.py
MCP_TRANSPORT=sse CODEREVIEW_MCP_PORT=8003 python autoops/mcp/mcp_codereview.py
MCP_TRANSPORT=sse MONITORING_MCP_PORT=8004 python autoops/mcp/mcp_monitoring.py
```
...
Install an ASGI server if needed:
...
```bash
pytest tests/unit/test_cloudwatch_agent.py
```

## Notes

- CloudWatch tools use the default boto3 credential chain.
- Monitoring tools currently include deterministic demo service profiles.
